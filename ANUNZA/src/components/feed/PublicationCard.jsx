...
/** Extrae rating de "[Reseña X/5] texto" */
...
export function PublicationCard({
  p, currentUserId, onToggleLike, onToggleFavorito, onDeletePublicacion, onOpenChat, onError,
}) {
  ...
  return (
    <article className="pub-card">
      ...
      <header className="pub-card-head">
        <img className="pub-avatar" src={p.autor_foto || 'https://api.dicebear.com/7.x/avataaars/svg?seed=anunza'} alt="" />
      ...
      {tags.length > 0 && (
        <div className="pub-tags">{tags.map((t) => (<span key={t} className="pub-tag">#{t}</span>))}</div>
      )}
      ...
      <footer className="pub-footer-bar">
        ...
        {/* Chat solo si NO es publicación propia */}
        {!isOwn && (
          <button type="button" className="pub-footer-btn" onClick={() => onOpenChat(p.usuario_id)} title="Abrir chat">
            <IconChat />
            <span>Chat</span>
          </button>
        )}
      ...
      {openComments && (
        <div className="pub-panel pub-panel-reveal">
              ...
              return (
                <li key={c.id} className="pub-comment">
                  ...
                  <div className="pub-comment-body">
                    ...
                    {/* like count */}
                    {(c.likes_count > 0) && (
                      <span className="pub-com-likes">{c.likes_count} {c.likes_count === 1 ? 'me gusta' : 'me gusta'}</span>
                    )}

                    {/* panel de reporte de comentario */}
                    {isReportingThis && (
          ...
          <div className="pub-comment-form">
            <input type="text" placeholder="Escribe un comentario…" value={draft} onChange={(e) => setDraft(e.target.value)}
      ...
      {openReport && (
        <div className="pub-panel pub-panel-reveal">
          ...
          <select value={reportMotivo} onChange={(e) => setReportMotivo(e.target.value)}
            ...
            <option>Contenido inapropiado</option><option>Spam o publicidad</option>
            <option>Información falsa</option><option>Acoso o amenazas</option><option>Otro</option>
          </select>
